...
mostly independent of the ones from E-12-11-003. Indeed, while this measurement 
...
significantly helping with nuclear effects.  This has interestingly an impact 
on both initial state and final state effects. In the initial state, the 
neutron is in fact not at rest and carries some Fermi momentum, which can be 
directly inferred from the kinematic of the spectator proton, thanks to the 
simple two body deuterium. On the final state side, detecting the spectator 
proton in a certain range of momentum and angle allows to significantly reduce 
the probability of final state interactions to have occurred.

In order to resolve the initial state issue, we evaluate the standard Lorentz 
invariant $x$ in terms of the spectator kinematics. $x$ acquires a star to 
indicate  true invariants rather than the values calculated assuming a 
stationary, on-shell target:
\begin{equation}
